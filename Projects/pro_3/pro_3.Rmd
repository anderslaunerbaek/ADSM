...
The property of the second stage in the model when using the `glmmTMB()` is that the random effects must be gaussian.
The model is given in (\ref{eq_2_1}) and the estimated parameters are given in (\ref{eq_2_2}).

\begin{equation}
\begin{aligned}
Y_{ i }|\lambda_{ ij }\sim& \mathcal{P}\left( \lambda _{ ij } \right)  \\
log\left( \lambda _{ ij } \right) =& X_i\beta+\gamma_{ij}\\
\gamma_{ ij } \sim& \mathcal{N} \left( u_{ij},\, \sigma^2_r \right) 
\end{aligned}
\label{eq_2_1}
\end{equation}
where $X_i$ is the design matrix with a constant $1$, the two regressors and their interaction. $i = 1,\,2,\, ...,\,300$ for each observation, $j = 1,\,2,\, ...,\,30$ for each group. $u_{ij}$ is the random effect from each group and $\sigma^2_r$ is the dispersion among the groups.
...
\begin{aligned}
Y_{ i }|\lambda_{ ij }\sim& \mathcal{P}\left( \lambda _{ ij } \right)  \\
log\left( \lambda _{ ij } \right) =& X_i\cdot `r paste0("\\begin{bmatrix}", paste(coef_Q3[-5], collapse = "\\\\"), "\\end{bmatrix}")`+\gamma_{ij}\\
\gamma_{ ij } \sim& \mathcal{N} \left( u_{ij},\, `r coef_Q3[5]` \right) 
\end{aligned}
\label{eq_2_2}
\end{equation}

where $u_{ij}$ is an estiamted vector whit `r length(fit_TMB_re)` elements assumed to follow the properties of Gaussianity.
```{r}
...
More less Random... not systematic behavoiur ewe wnat to model...
...
data.frame(fit_TMB = -fit_TMB$fit$objective,
           `Laplace_simulation` = -L) %>% 
  knitr::kable(., caption = "\\label{tb_2_4}Check of the accuracy.")
```
...
Now is the second stage model a gamma model with 


<!--
https://academic.oup.com/intqhc/article/15/4/319/1792979
-->

precision parameter $\lambda$ (the sampling distribution)


i.e., a two stage model. In the first stage a random mean value μ is selected
according to a Gamma distribution. The Y is generated according to a Poisson distribution with that value as mean value. 

\begin{equation}
\begin{aligned}
Y_{ i }|\mu_{ ij } \sim& \mathcal{P}\left(\mu_{ ij } \right)  \\
% log\left( \lambda _{ ij } \right) =& X_i\beta+\gamma_{ij}\\
\mu_{ij} \sim { G }\left( \sfrac { m }{ \gamma  } ,\, \gamma  \right) 
\end{aligned}
\label{eq_2_3}
\end{equation}
...
```{r, echo=TRUE}
# Negative log-likelihood function
nll_ga <- function(u, beta, X, sigma.u){
  eta <- X%*%beta + u[dat$group]
  # nll if fixed and random effects
  lln_fix <- -sum(dpois(dat$y,lambda = exp(eta),log=TRUE))
  # dgamma(x, shape, rate = 1, scale = 1/rate, log = TRUE)
  
  lln_ran <- -sum(dgamma(u, shape = 1, scale = sigma.u, log = TRUE))
...
nll_LA_ga <- function(theta, X){
    ...
    # message(paste(theta, collapse = ", "))
    est <- nlminb(rep(0,30), objective = nll_ga, beta=beta, sigma.u=sigma.u, X=X)
    u <- est$par
    l.u <- est$objective
    H <- hessian(func = nll_ga, x = u, beta = beta, sigma.u = sigma.u, X=X)
    # return log-likelihood
    return(l.u + 0.5 * log(det(H/(2*pi))))
}
```

using rounded estimated values `r paste0("(", paste0(round(fit_TMB$fit$par,0), collapse = ", "),")")` as initial starting value for the optimization..

```{r}

fit_LA_ga <- nlminb(round(fit_TMB$fit$par,0), nll_LA_ga, X=model.matrix(fit_TMB))
# fit_LA_ga <- nlminb(c(0,0,0,0,0), nll_LA_ga, X=model.matrix(fit_TMB))
...
```{r, eval=FALSE}
#names(coef_Q3) <- c("(Intercept)","x1","x2","x1:x2", "theta")
#names(coef_Q4) <- c("(Intercept)","x1","x2","x1:x2")
data.frame(fit_TMB = fit_TMB$fit$par, fit_LA_ga = fit_LA_ga$par) %>% 
  mutate(delta = fit_LA_ga-fit_TMB) %>% 
  knitr::kable(., row.names = c("(Intercept)","x1","x2","x1:x2", "theta"),
               caption = "\\label{tb_2_5}Comparison of parameter estimates by the glmmTMB method and the Laplace approximation metod.")
...
# importance sampling
nll_Laplace_simulate_ga <- function(theta, X, k, seed){
  set.seed(seed)
  beta <- theta[-5]
  sigma.u <- exp(theta[5]) #distribution of the U
  est <- nlminb(rep(0,30), objective = nll_ga, #initial value=0
                beta=beta, sigma.u=sigma.u, X=X)
  u <- est$par
  l.u <- est$objective #objective function = likelihood
  H <- diag(hessian(nll_ga,x=u, beta = beta, X=X, sigma.u=sigma.u))
  s <- sqrt(1/H)
  # do simulations
  L <- sapply(1:k, function(i) {
    u.sim <- rnorm(length(u), mean=u, sd=s)  
    # return log-likelihood
    return(exp(-nll_ga(u=u.sim,beta=beta,sigma.u=sigma.u,X=X))/prod(dnorm(u.sim,mean=u,sd=s)))
  })
  # return average negative log-likelihood
  return(-log(mean(L)))
}
...
```{r, eval=FALSE}
k <- 10000
L <- nll_Laplace_simulate_ga(fit_LA_ga$par,X=model.matrix(fit_TMB),k=k,seed=22)
#
data.frame(#fit_TMB = -fit_TMB$fit$objective,
